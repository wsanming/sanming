...
public class FileDownloadUtils {
    ...
    private static String baseDir =  FileUtils.path(Global.getUserfilesBaseDir("")); //
    ...
    public static void downloadFile2Zip(HttpServletRequest request, HttpServletResponse response, String [] names, String [] paths, String baseDir) {
        ...
        String directory = baseDir.substring(0,11)+"\\zipDir";
     ...
     * 下载单个文件
     */
    public static void downloadFile(HttpServletResponse response, String[] names, String[] paths){
        try {
            names[0] = URLDecoder.decode(names[0],"utf-8");
            paths[0] = URLDecoder.decode(paths[0],"utf-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        String rerealFilePath =getBaseDir()+paths[0];
        String realFileName =names[0];
        downloadFile(response,realFileName,rerealFilePath);
    }
